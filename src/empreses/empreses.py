...
from src.model_calls import call_bert_embedding_model
...
class Empresa:
    ...
    def query_vdb(self, query, min_len=10, max_len=250):
        return set([self.chunks['chunks'][i]['text'] for i in self.get_nns_by_vector(call_bert_embedding_model(query), 800) if len(self.chunks['chunks'][i]['text'].split(" ")) > min_len and len(self.chunks['chunks'][i]['text'].split(" ")) < max_len])
